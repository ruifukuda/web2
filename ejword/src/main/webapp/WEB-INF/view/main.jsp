...
<c:if test="${not empty ejw.list }">
	...
	<c:if test="${ejw.total gt ejw.limit }">
		...
		<c:if test="${ejw.pageNo gt 1 }">
			<li><a href="/ejword/main?searchWord=${ejw.searchWord }&mode=${ejw.mode }&page=${ejw.pageNo-1 }"><span>&larr;</span>前へ</a></li>
		</c:if>
		<c:if test="${ejw.pageNo * ejw.limit lt ejw.total }">
			<li><a href="/ejword/main?searchWord=${ejw.searchWord }&mode=${ejw.mode }&page=${ejw.pageNo+1 }"><span>&rarr;</span>次へ</a></li>
		</c:if>
		</ul>
	</c:if>	
